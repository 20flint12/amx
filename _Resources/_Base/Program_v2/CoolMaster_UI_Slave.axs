MODULE_NAME='CoolMaster_UI_Slave' (DEV vdvCM, DEV MVP[], INTEGER BUTTONS[])
...
{STRING:
	...
	CMD = Data.Text
		...
		SELECT
		{
		    ACTIVE(FIND_STRING(CMD,'ON' ,1)):{ON[MVP,BUTTONS[1]] OFF[MVP,BUTTONS[2]] send_string 0, "'On1'" POWER = 1}
		    ACTIVE(FIND_STRING(CMD,'OFF' ,1)):{ON[MVP,BUTTONS[2]] OFF[MVP,BUTTONS[1]] send_string 0, "'OFF1'" POWER = 0}
		    ACTIVE(FIND_STRING(CMD,'Cool' ,1)):{
						     SEND_COMMAND MVP,"'^BMF-',itoa(BUTTONS[3]),',0,%SW1'" // ПОКАЗАТЬ
						     ...
						     SEND_COMMAND MVP,"'^BMF-',itoa(BUTTONS[5]),',0,%SW0'" // СПРЯТАТЬ
                                                      }
		    ...
		    ACTIVE(FIND_STRING(CMD,'Heat' ,1)):{
						     SEND_COMMAND MVP,"'^BMF-',itoa(BUTTONS[5]),',0,%SW1'" // ПОКАЗАТЬ
						     ...
						     SEND_COMMAND MVP,"'^BMF-',itoa(BUTTONS[3]),',0,%SW0'" // СПРЯТАТЬ
						     }
